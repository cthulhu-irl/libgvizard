...
TEST_CASE("[attrtypes::EscString]")
{
  const EscString<std::string> escstring =
    "Graph: \\G, Node: \\N, Edge: \\E, Label: \\L, Head: \\H, Tail: \\T"_escstr;
  ...
  const char* expected_result =
    "Graph: A Graph, Node: A Node, Edge: An Edge, "
    "Label: A Label, Head: A Head, Tail: A Tail";

  REQUIRE(escstring.get_occurences().graph == 1);
  ...
  REQUIRE(escstring.get_occurences().sum() == 6);
